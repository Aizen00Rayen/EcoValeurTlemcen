...
export default function Home() {
  ...
  return (
    <>
      ...
      <section className="bg-kraft pb-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="fade-in text-center mb-10">
            <span className="inline-block bg-green-mid/10 text-forest text-sm font-semibold uppercase tracking-widest px-4 py-1.5 rounded-full mb-3">
              Notre impact
            </span>
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-forest">
              {lang === "ar" ? "قبل وبعد" : "Avant & Après"}
            </h2>
          </div>

          <div className="fade-in relative rounded-3xl overflow-hidden shadow-2xl group cursor-pointer">
            <img
              src="/pic5.jpeg"
              alt="Before and after neighbourhood transformation — Eco Valor Tlemcen"
              className="w-full h-72 sm:h-96 lg:h-[520px] object-cover object-center transition-transform duration-700 group-hover:scale-105"
            />
            {/* Gradient overlay */}
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 sm:p-10 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                <p className="text-white/70 text-sm uppercase tracking-widest mb-1 font-medium">
                  Eco Valor Tlemcen
                </p>
                <p className="text-white font-heading text-2xl sm:text-4xl font-bold leading-tight drop-shadow-lg">
                  {lang === "ar"
                    ? "نحوّل النفايات إلى موارد"
                    : "On transforme les déchets en ressources"}
                </p>
              </div>
              <Link
                to="/process"
                className="flex-shrink-0 inline-block bg-white text-forest font-semibold px-6 py-3 rounded-xl hover:bg-kraft hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 text-sm sm:text-base"
              >
                {lang === "ar" ? "اكتشف المراحل" : "Voir le processus"}
              </Link>
            </div>
          </div>
        </div>
